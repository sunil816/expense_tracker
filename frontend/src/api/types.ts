...
export type SuggestionField = 'Kind'
export type SuggestionState = 'Suggested' | 'Confirmed' | 'Rejected' | 'Edited'
export type SuggestionDecision = Exclude<SuggestionState, 'Suggested'>
...
export interface KindSuggestionDetails { suggestionId: string; previousKind: TransactionKind; suggestedKind: TransactionKind; resolvedKind: TransactionKind | null; state: SuggestionState; source: string; reason: string; suggestedAt: string; decidedAt: string | null }
export interface TransactionSuggestion { id: string; transactionId: string; field: SuggestionField; previousKind: TransactionKind | null; suggestedKind: TransactionKind | null; resolvedKind: TransactionKind | null; state: SuggestionState; source: string; reason: string; suggestedAt: string; decidedAt: string | null }
export interface TransactionSuggestionItem { suggestion: TransactionSuggestion; transaction: TransactionSummary }
export interface TransactionSuggestionPage { items: TransactionSuggestionItem[]; page: number; pageSize: number; totalCount: number }
export interface SavedTransaction { id: string; importPosition: number; sourceSequence: number | null; sourceFormat: SourceFormat; transactionDate: string; description: string; accountLabel: string | null; externalReference: string | null; direction: Direction; kind: TransactionKind; amount: number; currency: string; balanceAfter: number | null; categoryId: string | null; receiptUrl: string | null; lineExtractionStatus: LineExtractionStatus; duplicateFlag: DuplicateFlagDetails | null; kindSuggestion: KindSuggestionDetails | null }
